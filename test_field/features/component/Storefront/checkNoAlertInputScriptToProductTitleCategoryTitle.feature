@BaseCase @Admin @Storefront @checkNoAlertInputScriptToProductTitleCategoryTitle @fixture-shop
Feature: should not see any alert on storefront(set product/category)
  As a merchant
  ...
  @EAT-939 @EAT-939-1
  Scenario: should not see any alert on storefront(set product/category)
    ...
    Then 1st product of products list should contain '(dirty data)xss'
    Then alert should not be present
