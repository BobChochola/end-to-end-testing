@BaseCase @Admin @Storefront @CRM @CustomNotification @customUserRegisterNotificationSMSTraditionalChinese
@fixture-shop @fixture-mobile-signup-p2 @EAT-1018
Feature: Turn on sms user register notification toggle and check sms receive correctly（Traditional Chinese)
  As a merchant
  ...
  Scenario: User register send sms
    ...
    And I select sign up selection dropdown option included "使用手機號碼註冊" of sign up selection dropdown
    ...
    And I click on search button
    Then I should see row of mail content
    Then 1st row of mail content should contain sms zh-tw signup notification
