@BaseCase @Admin @Storefront @Order @OrderDelivery @customDeliveryCollectedSplitEditCheck @fixture-shop @fixture-product @fixture-payment @fixture-delivery
Feature: Custom delivery collected order check the split button
  As a merchant
  I can split the parent order when the order status is collected
  ...
  Scenario: Custom delivery collected order check the split button
    ...
    Then 1st row of order links should contain spilt order number
    When I click on 1st row of order links
    Then I should be redirected to Orders Show Page
