@BaseCase2 @Storefront @Admin @Order @Search @orderFiltersArrivedExportPackingSlip
...
Feature: Check order delivery status filters
 ...
 @C8252 @C8252-3
  Scenario: Add date filter(Arrived) and status filter then export the packing slip
    Given I am on Orders Page
    When I click on 1st row of order links
    Then I should be redirected to Orders Show Page
    And I select Arrived of delivery status dropdown
    ...
    Then 1st search label should contain "Delivery Status is Arrived"
    Then I should see row of order links
