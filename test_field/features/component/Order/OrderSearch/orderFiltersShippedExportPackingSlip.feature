@BaseCase2 @Storefront @Admin @Order @Search @orderFiltersShippedExportPackingSlip
...
Feature: Check order delivery status filters
  ...
  @C8252 @C8252-2
  Scenario: Add date filter(Shipped) and status filter then export the packing slip
    ...
    And I select delivery status filter dropdown option included "Shipped" of delivery status filter dropdown
    Then 1st search label should contain "Delivery Status is Shipped"
    Then I should see row of order links
    Then I should not see loading icon
    Then 1st row of delivery status should equal to "Shipped"
