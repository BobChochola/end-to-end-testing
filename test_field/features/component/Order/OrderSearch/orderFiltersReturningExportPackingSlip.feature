@BaseCase2 @Storefront @Admin @Order @Search @orderFiltersReturningExportPackingSlip
...
Feature: Check order delivery status filters
  ...
  @C8252 @C8252-6
  Scenario: Add date filter(Returning) and status filter then export the packing slip
    Given I am on Orders Page
    When I click on 1st row of order links
    Then I should be redirected to Orders Show Page
    And I select Returning of delivery status dropdown
    Then I should see confirm popover
    And I click on YES button
    Then delivery status should equal to "Returning"
    Given I am on Orders Page
    ...
    And I select delivery status filter dropdown option included "Returning" of delivery status filter dropdown
    Then 1st search label should contain "Delivery Status is Returning"
    Then I should see row of order links
    Then I should not see loading icon
    Then 1st row of delivery status should equal to "Returning"
