@BaseCase2 @Storefront @Admin @Order @Search @orderFiltersbyOrderStatusConfirmed
...
Feature: Check order filters
  ...
  Scenario: Add date filter and status filter then export the stock picking list
    ...
    And I select order status filter dropdown option included "Confirmed" of order status filter dropdown
    Then 1st search label should contain "Order status is Confirmed"
    Then I should see row of order links
