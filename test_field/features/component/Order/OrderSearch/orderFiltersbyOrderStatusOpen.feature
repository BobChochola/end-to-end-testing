@BaseCase2 @Storefront @Admin @Order @Search @orderFiltersbyOrderStatusOpen
...
Feature: Check order filters
  ...
  Scenario: Add date filter and status filter then export the stock picking list
    ...
    Given I am on Orders Page
    When I click on tab included 'Order is Open'
    Then search label should contain "Order status is Open"
    Then 1st row of order status should contain "Open"
    Then 1st row of order links should contain order number
