@BaseCase @Admin @Storefront @Order @CancelOrder @executeShipmentNotOtherCancelOrder-Other @fixture-shop @fixture-product @fixture-payment @fixture-delivery @fixture-promotion

Feature: exetuce shipment not other cancel order on Orders, one is cancelled the other is not.(family mart C2C)
  As a user of Shopline
  I want execute shipment cancel order on Orders Page
  one is cancelled, the other is not.
  ...
  @EAT-1163 @EAT-1163-2
  Scenario: exetuce shipment not other cancel order on Orders, one is cancelled the other is not.(family mart C2C)
    ...
    Then attention cancel order links should not contain order number 2
    And I click on execute others order button
    ...
    Then I should see additional filters button
    Then 1st row of order links should not contain order number
    Then 1st row of order links should contain order number 2
    And ---ROLL BACK---
    Given I am on Payment Setting Page
    When I click edit button of payment method list for the same order of row of payment method list included 'Cash on Delivery'
