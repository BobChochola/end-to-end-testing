@PromotionBaseCase @Admin @Promotion @CouponCode @sameTargetGroupsCanUseSamePromotionCoupon-Member @fixture-product @fixture-promotion @C25696
Feature: Same target groups can use promotion bundle for a same product auto and coupon - Member
  ...
  @EAT-1053 @EAT-1053-2
  Scenario: Same target groups can use promotion bundle for a same product auto and coupon - member
    Given I am on Promotion Create Page
    When I fill '(dirty data) Buy selected products over NT$100 get 90% off' to name field
    And I click on subtotal condition checkbox
    And I select minimum amount of condition type dropdown
