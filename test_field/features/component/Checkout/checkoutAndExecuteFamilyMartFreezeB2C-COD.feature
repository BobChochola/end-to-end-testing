@BaseCase @Admin @Storefront @Checkout @checkoutAndExecuteFamilyMartFreezeB2C-COD
@fixture-payment @fixture-delivery @fixture-product @fixture-promotion @fixture-shop @fixture-fmt-freeze
Feature: Family Mart Freeze B2C Pickup and Pay in Store Checkout and executed delivery
  As a guest
  ...
  @EAT-1284 @EAT-1284-2
  Scenario: Family Mart Freeze B2C Pickup and Pay in Store Checkout and executed delivery
    ...
    And I select delivery dropdown option included "Family Mart Freeze Pickup and pay in store (B2C)" of delivery method dropdown
    And I wait for 5 seconds for loading
    ...
    And I wait for 5 seconds for loading
    And I Refresh
    Then errors info should contain 'Failed transaction: order amount for：Family Mart Freeze Pickup and pay in store (B2C) should be 1 ~ NT$19,999'
    ...
    Then delivery detail should contain "Family Mart Freeze Pickup and pay in store (B2C)"
    Then payment detail should contain "Family Mart Freeze Pickup and pay in store (B2C)"
    ...
    Then delivery method should contain "Family Mart Freeze Pickup and pay in store (B2C)"
    Then payment type should contain "Family Mart Freeze Pickup and pay in store (B2C)"
    ...
    When I wait for 60 seconds sync
    Then I Refresh
    Then logistic service order status should contain 'Shippable'
    Then logistic service order status should contain 'Executable Shipping Deadline：before'
    Then logistic service order status should contain dateTomorrow
    Then delivery form should contain 'Family Mart Shipment Number'
    Then 3rd row of order activities logs should contain 'Get Shipment Number'
    Then 2nd row of order activities logs should contain 'Update Logistic Service Order Status As Processing'
    Then 1st row of order activities logs should contain 'Update Logistic Service Order Status As Request Accepted'
    ...
    Then mail detail should contain "Family Mart Freeze Pickup and pay in store (B2C)"
    When I click on view order button
    And I switch to 2 of tabs
    Then I should be redirected to Shop Orders Show Page
    And delivery detail should contain 'Family Mart Freeze Pickup and pay in store (B2C)'
    And payment detail should contain 'Family Mart Freeze Pickup and pay in store (B2C)'
    And ---ROLL BACK---
    Given I am on Promotion Index Page
    When I input "8th Promotion" to search field
