...
class ResourceManager {
  constructor({ file = null, resources = null, label }) {
    this.label = label
    this.tmpFilePath = '.'

    if (file) {
      this.allResources = require(file)
    }
    ...
    if (!this.allResources) {
      throw new Error('Resource Manager cant get resources')
    }
  }
  ...
  markInUse(resource) {
    return fs.writeFileSync(`${this.tmpFilePath}/${resource.handle}.${this.label}.resource.tmp`, 'true', 'utf8');
  }
  ...
  unmarkInUse(resource) {
    try {
      fs.unlinkSync(`${this.tmpFilePath}/${resource.handle}.${this.label}.resource.tmp`);
    } catch (err) { }
  }
  ...
  getAvailable(resources, inUse) {
    return resources.filter(resource => !inUse.includes(resource.handle))
  }
  ...
  getResource(tags, markInUse = true, retry = 10) {
