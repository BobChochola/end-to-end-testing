...
class PromotionCreatePage extends Page {
  ...
  get elements() {
    ...
    return {
      ...
      'minimum amount': this.by.css('ul[ng-class*="true"] a[title="Minimum Amount"]'),
      'minimum items': this.by.css('ul[ng-class*="true"] a[title="Minimum items"]'),
      ...
      'free gift search field': this.by.css('input[ng-model="searchInput"]'),

      // General Information > Select Products Eligible for Promotion
      'search by dropdown': this.by.css('.form-products-search-container selectbox.ng-isolate-scope:not(.visible-xs)'),
      'search by dropdown option': this.by.css('.form-products-search-container selectbox.ng-isolate-scope:not(.visible-xs) li'),
      ...
      'popup search dropdown': this.by.css('[ng-show="showSearchCriteria"]'),
      'popup search dropdown option': this.by.css('[ng-show="showSearchCriteria"] li'),
      ...
      'search dropdown option': this.by.css('div[class="input-group"] div[class="sb-container"] li'),
      ...
      'available date button': this.by.css('[ng-click="select(dt.date)"]:not([disabled])'),
      'now button': this.by.css('button[class="btn btn-sm btn-link ng-binding"][ng-disabled="isDisabled(\'now\')"]'),
      'add benefits button': this.by.css('i.fa.fa-plus-circle'),
      'select exclude product button': this.by.css('a.btn.btn-default.ng-binding'),
    }
  ...
  get samples() {
    ...
    return {
      ...
      'tomorrow start full time': `${this.props.sample.genDate(1)}` + " " + `${this.props.sample.genHour()+ ":00"}` + " " +`${this.props.sample.genTwelveHourPeriod()}`,
  ...
  get url() {
    ...
    return `${this.props.baseURL}/admin/${this.props.account.handle}/promotions/discount/new?locale=en`

  }

}

module.exports = PromotionCreatePage
